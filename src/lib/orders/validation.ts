...
export function validateCheckout(value: unknown): {data?:CheckoutInput; error?:string} {
 ...
 if(!Array.isArray(x.items)||!x.items.length||x.items.length>50||x.items.some(i=>!i.bookId||!i.slug||!["physical","ebook"].includes(i.format)||!Number.isInteger(i.quantity)||i.quantity<1||i.quantity>20)) return {error:"Your cart contains an invalid item."};
