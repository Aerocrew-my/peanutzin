...
export function Header({ articles = [], books = [], announcement = "KL International Book Fair 2026 · Hall 4, Booth 123" }: { articles?: Article[]; books?: Book[]; announcement?: string }) {
  ...
  useEffect(()=>{document.querySelectorAll<HTMLAnchorElement>('.site-header a[href="/admin/login"],.mobile-drawer a[href="/admin/login"]').forEach(link=>{link.href=signedIn?"/account":"/login";link.textContent=signedIn?"Account":"Login"})},[signedIn]);
