...
export function BookCover({ cover, title, author, sizes = "(max-width: 700px) 50vw, 22vw" }: { cover: string; title: string; author: string; sizes?: string }) {
  if (cover.startsWith("http")) return <span className="book-cover"><Image src={cover} alt={`Cover of ${title} by ${author}`} fill sizes={sizes} /></span>;
  return <span className="book-cover book-cover-fallback" role="img" aria-label={`PEANUTZIN preview cover for ${title} by ${author}`}><small>PEANUTZIN</small><strong>{title}</strong><i /><em>{author}</em></span>;
}
...
export function Header({ articles = [], books = [], announcement = "KL International Book Fair 2026 · Hall 4, Booth 123" }: { articles?: Article[]; books?: Book[]; announcement?: string }) {
  ...
  const [notice, setNotice] = useState(true); const [signedIn,setSignedIn]=useState(false); const { count } = useCart();
  useEffect(() => { if (localStorage.getItem("peanutzin-notice") === "hidden") document.documentElement.classList.add("notice-dismissed"); }, []);
  useEffect(()=>{const supabase=createClient();supabase.auth.getUser().then(({data})=>setSignedIn(Boolean(data.user)));const{data}=supabase.auth.onAuthStateChange((_event,session)=>setSignedIn(Boolean(session?.user)));return()=>data.subscription.unsubscribe()},[]);
...
export function Newsletter() {
  ...
  const [state,setState]=useState<"idle"|"loading"|"success"|"duplicate"|"error">("idle"),[message,setMessage]=useState("");
  async function submit(event: React.FormEvent) { event.preventDefault();setState("loading");setMessage("");try{const response=await fetch("/api/newsletter",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({email,source:"website"})}),data=await response.json();setMessage(data.message);setState(response.ok?(data.status==="duplicate"?"duplicate":"success"):"error");}catch{setMessage("We couldn’t subscribe you right now. Please try again.");setState("error");} }
  return <section className="newsletter" id="newsletter"><div><p className="eyebrow">The good stuff, occasionally</p><h2>Books, stories &amp; good trouble in your inbox.</h2><p>Occasional PEANUTZIN updates. No clutter.</p></div><form onSubmit={submit}><label htmlFor="newsletter-email">Email address</label><div><input id="newsletter-email" type="email" required value={email} onChange={(event) => setEmail(event.target.value)} placeholder="you@example.com" aria-describedby="newsletter-feedback"/><button className="button button-dark" disabled={state==="loading"}>{state==="loading"?"Subscribing…":"Subscribe"}</button></div><p id="newsletter-feedback" className={`newsletter-feedback ${state}`} role="status" aria-live="polite">{message}</p></form></section>;
}

export function Footer() { const about=[{label:"About Us",href:"/about"},{label:"Sell With PEANUTZIN",href:"/sell-with-us"},{label:"FAQ",href:"/faq"},{label:"Contact",href:"/contact"},{label:"Submissions",href:"/about#submissions"}],support=[{label:"Shipping & Returns",href:"/shipping-returns"},{label:"Terms of Service",href:"/terms"},{label:"Privacy Policy",href:"/privacy"}]; return <footer><div className="footer-main"><div><Link className="wordmark" href="/">PEANUTZIN<small>BOOK &amp; MEDIA | INDEPENDENT MALAYSIA</small></Link><p>A home for books, stories and new voices.</p></div><div><h3>Explore</h3>{navItems.slice(0,5).map((item)=><Link key={item.href} href={item.href}>{item.label}</Link>)}</div><div><h3>About</h3>{about.map((item)=><Link key={item.href} href={item.href}>{item.label}</Link>)}</div><div><h3>Support</h3>{support.map((item)=><Link key={item.href} href={item.href}>{item.label}</Link>)}<Link className="footer-secondary" href="/admin/login">Staff Admin</Link></div></div><div className="footer-bottom"><span>© 2026 PEANUTZIN</span><span className="social-links">Made with care in Malaysia · <a href="https://www.instagram.com/peanutzin/" target="_blank" rel="noopener noreferrer">Instagram</a> · <a href="https://www.facebook.com/peanutzin/" target="_blank" rel="noopener noreferrer">Facebook</a></span></div></footer>; }
...
export function PageShell({ title, eyebrow, children }: { title: string; eyebrow?: string; children: React.ReactNode }) { return <><Header /><main className="page-main container"><p className="eyebrow">{eyebrow ?? "PEANUTZIN"}</p><h1>{title}</h1>{children}</main><Newsletter/><Footer /><MobileNav /></>; }
