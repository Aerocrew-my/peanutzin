...
export async function saveArticle(_state: ActionState, form: FormData): Promise<ActionState> {
  ...
  try {
    ...
    const values = { title, slug, category: category as "news"|"gossip"|"event"|"feature", status: status as "draft"|"published", excerpt: nullable(form,"excerpt"), body: nullable(form,"body"), hero_image_path: newImage ?? nullable(form,"existing_image"), featured: form.get("featured") === "on", trending_rank: rank, published_at: published ?? null, event_start_at: category === "event" ? start ?? null : null, event_end_at: category === "event" ? end ?? null : null, event_location: category === "event" ? nullable(form,"event_location") : null, seo_title: nullable(form,"seo_title"), seo_description: nullable(form,"seo_description") };
    const supabase = await createClient();
